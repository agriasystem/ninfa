"""Robust statistics of the historical OTA shares (pure functions, `Decimal` only, no float).

The algorithm is EXACTLY the one of the Expected Engine and of the revenue/cost/labor detectors
(median, and P25/P75 by linear interpolation at index `(n - 1) * p`; IQR = P75 - P25), kept at
FULL PRECISION (the dedicated 50-digit context) and never rounded before a threshold is compared:
...
Because a share is a percentage, `upper_fence` can mathematically exceed 100: nothing here clamps
it, and the trigger logic does not need it clamped either.
...
from app.modules.intelligence.distribution.precision import CALCULATION_CONTEXT
from app.modules.intelligence.distribution.types import IQR_MULTIPLIER
...
def summarize(
    values: Sequence[Decimal], *, iqr_multiplier: Decimal = IQR_MULTIPLIER
) -> OtaShareStatistics:
    ...
    return OtaShareStatistics(median=median(values), p25=p25, p75=p75, iqr=iqr, upper_fence=fence)
