"""Robust statistics of the historical CPORs (pure functions, `Decimal` only, no float).

The algorithm is EXACTLY the one of the Expected Engine and of the revenue detectors (median, and
P25 / P75 by linear interpolation at index `(n - 1) * p`; IQR = P75 - P25), but a CPOR is not an
integer number of rooms, so the values are kept at FULL PRECISION (the dedicated 50-digit context)
and are never rounded before a threshold is compared:
...
There is NO outlier removal: the median and the quartiles are the robustness, and the fence only
asks that a month stands out of the historical distribution. Negative and zero CPORs (credit notes
can make a month's net cost <= 0) are ordinary values.
"""
...
from app.modules.intelligence.costs.types import CPOR_IQR_MULTIPLIER
...
def summarize(
    values: Sequence[Decimal], *, iqr_multiplier: Decimal = CPOR_IQR_MULTIPLIER
) -> CporStatistics:
    ...
    return CporStatistics(median=median(values), p25=p25, p75=p75, iqr=iqr, upper_fence=fence)
