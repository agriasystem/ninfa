...
The labor detector follows exactly the path of the revenue and cost detectors:

    CALCULATION VALUE  ->  THRESHOLD COMPARISON  ->  STATUS  ->  DISPLAY QUANTIZATION

Quotients (hours, a percentage, a coverage) are computed in the DEDICATED 50-digit context of
`revenue.precision`, independent of the process-wide `decimal` context, and are never rounded
before they are compared. The helpers are RE-EXPORTED, not copied, so the three detectors cannot
drift apart.
"""
